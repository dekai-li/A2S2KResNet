...
parser.add_argument(
    '-d', '--dataset', dest='dataset', default='WHU_Hi_LongKou', help="Name of dataset.")
...
parser.add_argument(
    '-i', '--iter', type=int, dest='iter', default=10, help="No of iter")
parser.add_argument(
...
def sampling_fixed(fixed_number, ground_truth):
    train = {}
    test = {}
    labels_loc = {}
    m = int(np.max(ground_truth))  # 类别总数
    for i in range(m):
        indexes = [
        ...
        nb_val = min(fixed_number, len(indexes))  # 若某类像素不足N个，最多取全部
...
def load_dataset(Dataset, split=0.01):
    data_path = 'dataset/'
    if Dataset == 'IN':
        ...
        SV = sio.loadmat(data_path + '2013Houston.mat')
        gt_SV = sio.loadmat(data_path + '2013Houston_gt.mat')
        data_hsi = SV['houston']
        gt_hsi = gt_SV['houston_gt']
        #K = data_hsi.shape[2]
        K = 30
        TOTAL_SIZE = 15029
        VALIDATION_SPLIT = split
        ...
        SV = sio.loadmat(data_path + 'xuzhou.mat')
        gt_SV = sio.loadmat(data_path + 'xuzhou_gt.mat')
        data_hsi = SV['xuzhou']
        gt_hsi = gt_SV['xuzhou_gt']
        #K = data_hsi.shape[2]
        K = 30
        TOTAL_SIZE = 68877
        VALIDATION_SPLIT = split
        ...
        mat_data = sio.loadmat(data_path + 'WHU_Hi_HanChuan.mat')
        mat_gt = sio.loadmat(data_path + 'WHU_Hi_HanChuan_gt.mat')
        data_hsi = mat_data['WHU_Hi_HanChuan']
        gt_hsi = mat_gt['WHU_Hi_HanChuan_gt']
        #K = data_hsi.shape[2]
        K = 30
        TOTAL_SIZE = 257530
        VALIDATION_SPLIT = split
        ...
        mat_data = sio.loadmat(data_path + 'WHU_Hi_LongKou.mat')
        mat_gt = sio.loadmat(data_path + 'WHU_Hi_LongKou_gt.mat')
        data_hsi = mat_data['WHU_Hi_LongKou']
        gt_hsi = mat_gt['WHU_Hi_LongKou_gt']
        #K = data_hsi.shape[2]
        K = 30
        gt = gt_hsi.reshape(-1)
        TOTAL_SIZE = np.count_nonzero(gt)
        VALIDATION_SPLIT = split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
        ...
        raise ValueError(f"不支持的格式: {Dataset}")
...
lr, num_epochs, batch_size = 0.0005, 200, 64
...
class S3KAIResNet(nn.Module):
    def __init__(self, band, classes, reduction):
        ...
        self.batch_norm2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True))
        self.conv3 = nn.Conv3d(
            in_channels=128,  # 与 x2.shape[1] 对齐
            out_channels=PARAM_KERNEL_SIZE,
            kernel_size=(1, 3, 3),
            padding=(0, 1, 1),
            stride=(1, 1, 1))
...
summary(
    model,
    input_size=(1, 1, BAND, img_rows, img_cols),  # (batch, C, D, H, W)
    device=device,
    verbose=1
)
def train(net,
    ...
    for epoch in range(epochs):
        ...
        PATH = "project_result/net_DBA.pt"
...
for index_iter in range(ITER):
    ...
    train_indices, test_indices = sampling_fixed(30, gt)
    ...
    with torch.no_grad():
        for X, y in test_iter:
            ...
            net.eval()
            y_hat = net(X)
            pred_test.extend(net(X).cpu().argmax(axis=1).numpy())
    toc2 = time.time()
    ...
    torch.save(
        net.state_dict(), "project_result/" + str(img_rows) + '_' +
...
record.record_output(
    ...
    'project_result/' + 'S3KAIResNetpatch' + str(img_rows) + '_' + Dataset + 'split'
...
Utils.generate_png(
    ...
    'project_result/' + 'S3KAIResNetpatch' + str(img_rows) + '_' +
